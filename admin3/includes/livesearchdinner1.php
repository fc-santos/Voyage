...
if (strlen($q)>0) {
    ...
    if ($r != '') {
        ...
        if ($idLieu != null) {
            $sql2 = "SELECT * FROM manger WHERE nom LIKE '%".$q."%' AND idLieu = " . $idLieu;
        ...
        $sql2 = "SELECT * FROM manger WHERE nom LIKE '%".$q."%'";
        $result = $conn->query($sql2);
        $liste = '';
    }
    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_object()) {
            if ($row->nom != null) {
                $liste = $liste . '<p onclick="prendreLaValeur(this,\'livesearchdinner1\',\'#dinner1\')">' . utf8_encode($row->nom) . '</p>';
            }
        }
    } else {
        $liste = '';
